...
-- Displays text that says correct. / incorrect.
local correct 
local incorrect
...
local function LoseScreenTransition( )        
    composer.gotoScene( "you_lose", {effect = "flipFadeOutIn", time = 1000})
end 

-- func5tion that calls win screen if has three correct answers
local function WinCaller( )
...
local function TouchListenerAnswer(touch)
    ...
    if (touch.phase == "ended") and (alreadyClickedAnswer == false) then
        ...
        if (answer == tonumber(userAnswer)) then     
            ...
            timer.performWithDelay( 1000, WinCaller )
        end        
...
local function PlayBkgSound()
    timer.performWithDelay(14000, PlayBkgSound)
    -- make bkg sound
...
function scene:create( event )
    ...
    -- create text that displays inccorect if user answers the question wrong
